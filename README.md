...
- **Own-device connections** — Bluetooth devices this phone pairs with or connects to, and Wi-Fi networks it joins. (Deliberately does *not* scan for or log other nearby devices/networks the phone never connects to — that would be about detecting who else is nearby, a different and more invasive kind of monitoring than the rest of this app does.)
...
| `SetupScreen` / `TimelineScreen` / `PauseScreen` / `ReportScreen` | `ui/` | Compose UI: permission + PIN onboarding, the event timeline (view photos, play memos, record new ones), the pause gate, and report/backup generation |
